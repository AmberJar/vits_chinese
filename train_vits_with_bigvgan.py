...
def run(rank, n_gpus, hps):
    ...
    try:
        _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.resume, "G_*.pth"), net_g,
                                                        optim_g)
        _, _, _, epoch_str = utils.load_checkpoint(utils.latest_checkpoint_path(hps.resume, "D_*.pth"), net_d,
                                                        optim_d)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid) in enumerate(train_loader):
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(x, x_lengths, spec, spec_lengths, sid)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        ...
        y_hat, attn, mask, *_ = generator.module.infer(x, x_lengths, sid=sid, max_len=1000)
    ...
    audio_dict = {
      "gen/audio1": y_hat[0,:,:y_hat_lengths[0]],
      "gen/audio2": y_hat[1,:,:y_hat_lengths[1]],
      "gen/audio3": y_hat[2,:,:y_hat_lengths[2]],
      "gen/audio4": y_hat[3,:,:y_hat_lengths[3]],
    }
    if global_step == 0:
